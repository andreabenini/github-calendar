...
module.exports = function GitHubCalendar (container, username, options) {
    ...
    let fetchCalendar = () => options.getCalendar(username).then(body => {
        ...
        // Remove GitHub's visible "Skip to contributions year list" link (and similar
        // skip links used for small screens). On GitHub this link is visually hidden
        // by their CSS, but when injecting the raw HTML it can become visible.
        if (cal) {
            for (const a of Array.from(cal.querySelectorAll('a'))) {
                const txt = a.textContent && a.textContent.trim()
                const href = a.getAttribute && a.getAttribute('href')
                // exact match text or a small-screen skip link (show-on-focus / year-link)
                if (txt && txt.includes('Skip to contributions year list')) {
                    a.remove()
                    continue
                }
                if (a.classList && (a.classList.contains('show-on-focus') || a.classList.contains('hide-sm') || a.classList.contains('hide-md') || a.classList.contains('hide-lg'))) {
                    // only remove if it's an internal anchor to a year-link
                    if (href && href.indexOf('#year-link') === 0) a.remove()
                }
            }
        }
